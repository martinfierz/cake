...
/*									*/
/* Incorrect result returned because DBWHITE can capture:			*/
/* 									*/
/* 8  . - . - . - . -							*/
/* 7  - . - . - . - .							*/
/* 6  . - . - . - . -							*/
/* 5  - . b . b . - .   DBWHITE to move		*/
/* 4  . w . - . - . -							*/
/* 3  w . - . - . - .							*/
/* 2  . - . - . - . -							*/
/* 1  - . - . - . - .							*/
...
/* DBBLACK, then a capture would be possible.				*/
/* 8  . - . - . - . -							*/
/* 7  - . - . - . - .							*/
/* 6  . - . b . - . -							*/
/* 5  - . b . - . - .   DBWHITE to move		*/
...
typedef struct {
	long		rangeBK;     /* number of DBBLACK king configurations */
	long		rangeWK;     /* number of DBWHITE king configurations */
	long		firstBPIdx;
	long		nextBPIdx;
	long		numPos;
	sidxindexptr	dbptr;
...
int DBInit()
{
	...
	DBENTRYPTR ptr;
	short int * dbbufptr;
   char dbfilename[80]="no endgame database installed",dbindexfilename[80]="no index file";
   int founddb=0;
   int buffs;
...
/*   fp=fopen("db.ini","r");
   ...
   if(fp!=NULL)
      ...
      if(fp!=0)
         ...
         fclose(fp);
         founddb=1;
         sprintf(dbfilename,"db6");
         sprintf(dbindexfilename,"db6.idx");
         DBPieces=6;
         buffs=1000;
         }
      }
   if(founddb==0)
   	...
   	if(fp!=NULL)
      	...
      	if(fp!=0)
         	...
         	fclose(fp);
         	founddb=1;
         	sprintf(dbfilename,"db4");
         	sprintf(dbindexfilename,"db4.idx");
         	DBPieces=4;
         	buffs=220;
         	}
         }
...
/* on a unix-machine this is different!? */
   ...
   if(logging&2)
	...
	/*			Most  positions in DB are DBBLACK wins	*/
	/*								*/
	...
	buffers += buffs;
	if(logging&1)
   	{fprintf(fp,"... allocating %d buffers\n", buffers);fflush(fp);}
	if(logging&2)
...
long DBLookup(struct pos p, int turn)
{
   ...
   Gposition=p;
   /*Turn=DBBLACK;
   /* in my version this database is only called with Turn=DBBLACK */
   ...
   /* we don't need to hand on position and turn since they're global */
	...
	if (start != end)
		...
		if(logging&1)
			{
         fprintf(fp,"ERROR: cannot agree %d %d \n", start, end);
         fflush(fp);
         }
		if(logging&2)
			{
	...
	if(logging&1)
      {fprintf(fp,"ERROR: scanned block and did not find\n");
		fprintf(fp,"INDEX %d CINDEX %d\n", index, cindex);
		fprintf(fp,"Start %d end %d byte %d = middle %d\n",
			dbentry->startaddr,
			dbentry->endaddr,
	...
	long PIECES;
{
	...
	/*
	 * Compute the number of DBBLACK and DBWHITE king configurations for this
	 * database slice
	 */
 ...
 * Example:
 *
 *	Suppose there are DBBLACK kings on squares 1 and 11, and DBWHITE kings
 ...
 * index of the DBWHITE checkers (since, unlike the other piece types, they are
...
/*
 * SquishBP - squish the DBBLACK checker configuration and determine its
 *	      reverse index
 *
 * Parameters:
 *
 *	nXX	number of DBBLACK checkers
 *	XXpos	array containing square numbers (in descending order)
 *
 * Since the DBBLACK checkers are placed on the board first, there are no
...
/*
 * SquishWP - squish the DBWHITE checker configuration and determine its
 *	      reverse index
 *
 * Parameters:
 *
 *	nXX	number of DBWHITE checkers
 *	nYY	number of DBBLACK checkers
 *	XXpos	array containing DBWHITE checker square numbers
 *	YYpos	array containing DBBLACK checker square numbers
 *
 * Since the DBWHITE checkers are placed on the board after the DBBLACK checkers,
 * there may have been possible conflicts with the DBBLACK checkers during the
 * enumeration process.  As a result, the computation applied directly to the
 * DBBLACK checker configuration cannot be applied to the DBWHITE checker
 * configuration without first "squishing" the DBWHITE checker configuration.
 * This is done by determining how many of the DBBLACK checkers interfere with
 * each DBWHITE checker, and then decrementing this amount from each DBWHITE
...
/*
 * SquishBK - squish the DBBLACK king configuration and determine its
 *	      reverse index
 *
 * Parameters:
 *
 *	nXX	number of DBBLACK kings
 *	nYY	number of DBWHITE checkers
 *	nZZ	number of DBBLACK checkers
 *	XXpos	array containing DBBLACK king square numbers
 *	YYpos	array containing DBWHITE checker square numbers
 *	ZZpos	array containing DBBLACK checker square numbers
 *
 * Since the DBBLACK kings are placed on the board after the DBWHITE checkers,
 * there may have been possible conflicts with both the DBWHITE checkers and
 * the DBBLACK checkers during the enumeration process.  As with the DBWHITE
 * checkers, the DBBLACK king configuration must first be "squished" to
 * determine its correct index.  This is done by determining how many of
 * the DBWHITE and DBBLACK checkers interfere with each DBBLACK king, and then
 * decrementing this amount from each DBBLACK king square.  After this is
...
/*
 * SquishWK - squish the DBWHITE king configuration and determine its
 *	      reverse index
 *
 * Parameters:
 *
 *	nXX	number of DBWHITE kings
 *	nYY	number of DBBLACK kings
 *	nZZ	number of DBWHITE checkers
 *	nYZ	number of DBBLACK checkers
 *	XXpos	array containing DBWHITE king square numbers
 *	YYpos	array containing DBBLACK king square numbers
 *	ZZpos	array containing DBWHITE checker square numbers
 *	YZpos	array containing DBBLACK checker square numbers
 *
 * Since the DBWHITE kings are placed on the board after the DBBLACK kings,
 * there may have been possible conflicts with the DBBLACK kings, DBWHITE
 * checkers, and the DBBLACK checkers during the enumeration process.  As
 * with the DBWHITE checkers and DBBLACK kings, the DBWHITE king configuration
 * must first be "squished" to determine its correct index.  This is done
 * by determining how many of the DBBLACK kings, DBWHITE checkers, and DBBLACK
 * checkers interfere with each DBWHITE king, and then decrementing this
 * amount from each DBWHITE king square.  After this is complete, the reverse
 * index is computed.
 */
...
/*
-- In: a DBBLACK-to-move board in Locbv[]
...
int32 dbLocbvToSubIdx(DBENTRYPTR *dbentry,struct pos p,int turn)
{
	...
	/* If it's DBWHITE to move, reverse the board & look up DBBLACK to move. */
	if (Turn == DBWHITE) {

   	/* additional code by MF*/
      Locbv[DBWHITE]=Gposition.wm|Gposition.wk;
      Locbv[DBBLACK]=Gposition.bm|Gposition.bk;
	...
	*dbentry = DBTable[DBINDEX(nbk, nwk, nbp, nwp)];
	 ...
	 *
	 * where:
	 *       (1) is the number of positions with the DBBLACK checker index
	 *	     less than the desired DBBLACK checker index
	 *       (2) is the number of positions with the desired DBBLACK checker
	 *	     index but having a DBWHITE checker index less than the
	 *	     desired DBWHITE checker index
	 *       (3) is the number of positions with the desired DBBLACK and
	 *	     DBWHITE checker index but having a DBBLACK king index less
	 *	     than the desired DBBLACK king index
	 *	 (4) is the number of positions with the desired DBBLACK checker,
	 *	     DBWHITE checker, and DBBLACK king index but having a DBWHITE
	 *	     king index less than the desired DBWHITE king index
	 */
...
/*
 * A database slice is determined by the leading rank of the DBBLACK and DBWHITE
 * checkers.  A position index within the slice is computed by the position
 * of the DBBLACK checkers, then adding the DBWHITE checkers, followed by the
 * DBBLACK kings, and finally the DBWHITE kings.
 *
 * The following "|" notation refers to "choose";
 *  i.e. | n | = n!/((n-k)!k!).
 *       | k |
 *
 * The number of positions with nbp DBBLACK checkers with leading rank rbp
 * (where 0 <= rbp <= 7) is MaxBP = | 4 x (rbp+1) | - | 4 x rbp |.
 *                                  |     nbp     |   |  nbp    |
 *
 * Next the DBWHITE checkers are added.  This is discussed in more detail below.
 *
 * The number of positions created by adding nbk DBBLACK kings is
 *      MaxBK = | 32 - nbp - nwp |
 *              |      nbk       |
 *
 * and the number of positions created by adding nwk DBWHITE kings is
 *      MaxWK = | 32 - nbp - nwp - nbk |.
 *              |         nwk          |
 *
 *
 * Computing the number of positions with nbp DBBLACK checkers having leading
 * rank rbp and nwp DBWHITE checkers having leading rank rwp is difficult because
 * some of the DBBLACK checker configurations may have one or more checkers
 * within the first rwp checker squares.  Thus the simple formula applied to
 * computing MaxBP cannot be applied to MaxWP because different DBBLACK checker
 * configurations may have a different number of DBWHITE checker configurations.
 * MaxWP is computed by creating a set of subindexes for every DBBLACK checker
 * configuration.  The number of DBWHITE checker positions for each DBBLACK checker
 * configuration is computed and stored, as well as a cumulative total.  The
 * amount of storage required can be quite large for a large number of DBBLACK
 * checkers.
 *
...
void sidxCreate(DB_REC_PTR_T p,long nBP,long nWP,long rankBPL,long rankWPL,
	...
	if (nBP == 0) {
		...
		*nextBPLIdx = 1;
		...
		/*
		 * Determine the number of DBBLACK positions which have one
...
#ifdef UNUSED
		/*
		 * Don't fill the entire rank with DBBLACK pawns; leave room
		 * for one DBWHITE pawn.
		 */
	 ...
	 * cumulative number of DBWHITE checker positions for each DBBLACK
	...
	/*
	 * This array holds the starting index for each DBWHITE checker
	...
	/* Do, for each DBBLACK checker configuation... */
	for (Maximum = *nextBPLIdx - *firstBPLIdx; Maximum; Maximum--) {
		if (nWP) {
			/* Compute available squares for DBWHITE checkers */
			if (nBP) {
			 ...
			 * DBWHITE checker positions for this DBBLACK checker
			 * configuration
			 */
			...
			/*
			 * Save the starting index for this DBWHITE checker
		...
		/*
		 * Add the number of DBWHITE checker positions to the cumulative
		 * amount
		 */
		Base += (nextWPLIdx - firstWPLIdx);

		/* Get the next DBBLACK checker configuration */
		if (nBP)
			NextIndex(BPBOARD);
	}
	/* Restore the original DBBLACK checker configuration */
	if (nBP)
		LoadIndex(BPBOARD);
	*numPos = Base;
} /* sidxCreate() */
...
/*
 * Nsq - return the number of squares available for placement of the DBWHITE
 * checkers.
 *
 * Paramters:
 *	rank	leading DBWHITE checker rank
 *	Ppos	an array describing the square number of the DBBLACK checkers
 *	nP	is the number of DBBLACK checkers.
 *
 * Although this routine is generalized, it is used only for determining DBWHITE
 * checker squares.
 */
 ...
 *
 *		BoardPos[0] => DBBLACK checkers (BPBOARD)
 *		BoardPos[1] => DBWHITE checkers (WPBOARD)
 *		BoardPos[2] => DBBLACK kings (BKBOARD)
 *		BoardPos[3] => DBWHITE kings (WKBOARD)
 *
 * All square numbers for the Kings and DBBLACK checkers are stored in increasing
 * numerical order, from left to right.  The DBWHITE checker square numbers are
 ...
 * suppose three DBBLACK checkers occupy database squares 0,1,2 (which correspond
 ...
 * Example:
 *
 *	Suppose DBBLACK checkers are on squares 1, 3 and 4.  The enumeration
 *	order proceeds as follows: (0,1,2) (0,1,3) (0,2,3) (1,2,3) (0,1,4)
 *	(0,2,4) (1,2,4) (0,3,4) (1,3,4).  Thus the reverse index for the DBBLACK
 ...
 * the index of the DBWHITE checkers.
 */
long DBindex(long *pos,long k) {
	register long offset = 0;
