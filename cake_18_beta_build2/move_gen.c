...
int getorderedmovelist(POSITION *p, MOVE movelist[MAXMOVES])
	{
...
int numberofmoves(MOVE m[MAXMOVES])
	{
	int i;
...
int makemovelist(SEARCHINFO *si, POSITION *p, MOVE movelist[MAXMOVES],int values[MAXMOVES], int bestindex, int32 killer)
	{
...
void blackorderevaluation(SEARCHINFO *si, POSITION *p,MOVE ml[MAXMOVES],int values[MAXMOVES],int n)
	{
	int eval;
	int32 from,to;
	int32 black;
...
void whiteorderevaluation(SEARCHINFO *si, POSITION *p,MOVE ml[MAXMOVES],int values[MAXMOVES],int n)
	{
	int eval;
	int32 from,to;
	int32 white;
...
int makecapturelist(SEARCHINFO *si, POSITION *p,MOVE movelist[MAXMOVES],int values[MAXMOVES], int32 bestindex)
	{
	...
	*/
	...
	if(p->color==BLACK)
		...
		if(n>1)
			...
			for(i=0; i<n; i++)
				{
				values[i] = MANCAPT*bitcount(movelist[i].wm);
				values[i] += KINGCAPT*bitcount(movelist[i].wk);
				
				// check promotion
				if(movelist[i].bm && movelist[i].bk)
					values[i]+=PVVAL;
				// get tosquare for manmove
				/*tmp = movelist[i].bm & free;
				if(tmp & CENTER)
					values[i] += MCV;*/
...
#ifdef MOHISTORY
/*				
				from=(movelist[i].bm|movelist[i].bk)&black;
				to=(movelist[i].bm|movelist[i].bk)&(~black);

				if(si->hashstores>MINHASH) 
					values[i] +=( (HISTORY*history[LSB(from)][LSB(to)]) / (si->hashstores));  // vtune: if is loopindependent - take out
*/
#endif
		...
		if(n>1)
			...
			for(i=0; i<n; i++)
				{
				values[i] = MANCAPT*bitcount(movelist[i].bm);
				values[i] += KINGCAPT*bitcount(movelist[i].bk);
				if(movelist[i].wm && movelist[i].wk)
					values[i]+=PVVAL;
				}
			}
#ifdef MOHASH
