...
int bookgen(OLDPOSITION *q, int color, int *numberofmoves, int values[MAXMOVES], int how, int depth, double searchtime)
	...
	n = makecapturelist(&si, &p, movelist, values, 0);
	if(!n)
...
int analyze(POSITION *p, int color, int how, int depth, double searchtime, int *stat_eval)
	...
	staticeval = evaluation(p,&(si.matcount),0,&dummy,0,maxNdb,&e);
...
int firstnegamax(SEARCHINFO *si, POSITION *p, MOVE movelist[MAXMOVES], int d, int alpha, int beta, MOVE *best)
	...
	*best = movelist[0];
	...
	n = makecapturelist(si, p, ml2, statvalues, 0);
...
#endif	
	hashstore(si, p, bestvalue, alpha, d, best, bestindex, iscapture[si->realdepth]);
#ifdef THREADSAFEHT
...
int csearch(SEARCHINFO *si, POSITION *p, int alpha, int beta)
	...
	if(testcapture(p) == 0)
		return evaluation(p, &(si->matcount), alpha, &delta, 0, maxNdb, &e);	
	
	n = makecapturelist(si, p, movelist, values, forcefirst);
...
int negamax(SEARCHINFO *si, POSITION *p, int d, int alpha, int32 *protokiller, int *bestmoveindex, int truncationdepth, int truncationdepth2, int iid)
	...
	normaleval = evaluation(p,&(si->matcount),alpha,&delta, stmcapture|sntmcapture, maxNdb, &e);
	reverseeval = evaluation(&q, &revmc, -alpha, &delta, stmcapture|sntmcapture, maxNdb, &re);

	if(normaleval != reverseeval)
		{
		printboard(p);
		printf("\nnormal: %i\treversed: %i", normaleval, reverseeval);
		...
		printf("\nrevers: %i br, %i comp, %i cramp, %i hold, %i king, %i king-man, %i material, %i men, %i RA, %i ST", re.backrank, re.compensation, re.cramp, re.hold, re.king, re.king_man, re.material, re.men, re.runaway, re.selftrap);
		getch();
		//printf("!");
		}
	//else
...
// or else it's dangerous to call evaluation if side not to move may have a capture
	...
	if(!stmcapture && !isdbpos && !ispvnode)
		...
		localeval = evaluation(p,&(si->matcount),alpha,&delta, stmcapture|sntmcapture,maxNdb, &e);
#ifdef SUPERPRUNE
		issuperprune = delta & SUPERPRUNECONST;
#endif		
		/*if(issuperprune)
			{
			printboard(p);
	...
	if(d<=0 && !stmcapture && !iid && (safemovenum == 0)) // SAFE
		...
		if(!sntmcapture)
			...
			if(localeval != MATE)
				...
				value = evaluation(p,&(si->matcount),alpha,&delta,0,maxNdb, &e);
			...
			if(localeval != MATE && ((localeval > alpha) || (localeval < alpha-QSEARCHLEVEL)))
				...
				if(localeval == MATE)
					{
					localeval = evaluation(p,&(si->matcount),alpha,&delta,0,maxNdb,&e);
			...
			if(!isdbpos)
				...
				if(localeval != MATE)
					...
					value = evaluation(p,&(si->matcount),alpha,&delta,1,maxNdb, &e);
	...
	// razoring
#ifdef RAZORING
	if(d <= FRAC && !stmcapture && !sntmcapture && !iid && (safemovenum == 0)) 
		{
		// check that there are no potential kings to be made here
		if(((p->color == BLACK) && ((p->bm&RANK7) == 0)) || ((p->color == WHITE) && ((p->wm&RANK2) == 0)))
			{
			// we are near the branch end, and there is no capture for either side - 
			// if the eval is far enough from the window, we just return the evaluation
			if(localeval != MATE)
				value = localeval;
			else
				value = evaluation(p,&(si->matcount),alpha,&delta,0,maxNdb, &e);

			value = max(value, qsearch(si, p, alpha, alpha+1, 0));
			if(value < alpha-RAZORWINDOW || value > alpha + RAZORWINDOW)
				return value;
			}
		}
#endif // RAZORING
	...
	*bestmoveindex = bestindex;
	...
	if(d>=0)
		...
		EnterCriticalSection(&hash_access);
#endif	
		hashstore(si, p, maxvalue, alpha, d, &movelist[bestindex], bestindex, stmcapture); 
#ifdef THREADSAFEHT
		LeaveCriticalSection(&hash_access);
#endif	
		}
...
void hashstore(SEARCHINFO *si, POSITION *p, int value, int alpha, int depth, MOVE *best, int32 bestindex, int iscapture)
	{
	...
	//if(iscapture == 0)
		...
		if(p->color==BLACK)
			{
			// warning: if this is a "circular jump" with a king, then we get both from and to 0!
...
#ifndef ALWAYSSTORE
	// reduce mindepth by OVERWRITEDEPTH, so that slightly more valuable entries can be overwritten,
	// but not really valuable entries
	if(mindepth - OVERWRITEDEPTH >(depth)) 
		return;
#endif
...
//	int staticeval;
	...
	for(i=0;i<MAXDEPTH;i++)
		...
		n = makecapturelist(si, &Lp,movelist,values,forcefirst);
		if(n)
			capture = 1;
		else 
			capture = 0;
...
int booklookup(POSITION *p, int *value, int depth, int32 *remainingdepth, int *best, char str[256])
	...
	*/
	...
	if(bookfound)
		...
		n = makecapturelist(&s, p, ml, dummy, 0);
		if(!n)
