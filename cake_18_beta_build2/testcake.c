...
int doelevenman(void)
	...
	return 0;
	// open file for test result
	fpout = fopen("FEN.txt","w");
	
	// open file test.txt with FEN strings,
	// read file line by line
	fp = fopen("C:\\11man.txt","r");
	if(!fp)
		return 0;
	...
	while (!feof(fp))
		...
		if(str[0]=='[' && str[1]=='F')
			{
			// str now holds a not-so-nice fen string
			niceFEN = str+6;
			for(i=0; i<100; i++)
				{
				if(niceFEN[i] == '"')
