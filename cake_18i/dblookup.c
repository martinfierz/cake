		...
		for(nb=4;nb<=4;nb++)
			...
			for(bk=0;bk<=nb;bk++)
				...
				for(wk=0;wk<=nw;wk++)
					...
					sprintf(dbname,"db%i_%i%i%i%i.cpr",bm+bk+wm+wk,bm,bk,wm,wk);
		...
		for(nb=maxpieces-maxpiece;nb<=maxpiece;nb++)
			...
			for(bk=0;bk<=nb;bk++)
				...
				for(wk=0;wk<=nw;wk++)
					...
					sprintf(dbname,"db%i_%i%i%i%i.idx",bm+bk+wm+wk,bm,bk,wm,wk);
					sprintf(out,"parsing %s",dbname);
					fp = fopen(dbname,"rb");
					if(fp != NULL)
						{
