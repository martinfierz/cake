...
int hashreallocate(int newMB)  // call with hashmegabytes
	...
	hashmegabytes = newMB;
	newsize = (hashmegabytes)*(int)1048576/sizeof(HASHENTRY);
	...
	while(pointer == NULL) {
		...
		pointer = (HASHENTRY*)malloc((newsize + (int)HASHITER) * sizeof(HASHENTRY) + 64);
...
void hashstore(SEARCHINFO *si, POSITION *p, int value, int alpha, int depth, MOVE *best, int32 bestindex)
	...
	// update history table 
		if (p->color == BLACK)
		{
			...
			si->history[LSB(from)][LSB(to)] += (depth); // +FRAC);
		}
		else
		{
