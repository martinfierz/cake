	...
	if(ml[i].bm)
        ...
        if(from & C3)
			...
			if(from & 0xFF)
				...
				eval -= blackbackrankeval[p->bm & 0xFF] / 2;
				eval += blackbackrankeval[ (p->bm ^ ml[i].bm) & 0xFF] / 2;
				}
			else
				{
				if(from&C4)
         ...
         /* man going down the board */
			...
			if(from&C3)
			...
			else
				{
      			if(from & 0xFF000000)
					{
					eval -= whitebackrankeval[p->wm >> 24] / 2;
					eval += whitebackrankeval[ (p->wm ^ ml[i].wm) >> 24] / 2;
					}
				else if(from&C4)
      				eval -= MANC4VAL;
				}
...
int makecapturelist(POSITION *p,MOVE movelist[MAXMOVES],int values[MAXMOVES], int32 bestindex)
	{
