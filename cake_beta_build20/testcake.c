...
main()
	...
	testfile = fopen("c:\\programme\\checkerboard\\testcake.txt","r");
	if(testfile == NULL)
	{
	...
	for(n=0; n<MAXTESTPOS; n++)
		...
		fprintf(fp2,"\n%2i \t%3.1f \t%3.1f",i,(float)newnodes[n][0]/(float)oldnodes[n][0]*100,(float)newnodes[n][1]/(float)oldnodes[n][1]*100);
		fflush(fp2);
	...
	sprintf(str,"\n\nRatio for depth 17: %2.2f",r17);
	fprintf(fp2,"%s",str);
	printf("%s",str);
	sprintf(str,"\n\nRatios for depth 21: %2.2f",r21);
	fprintf(fp2,"%s",str);
	printf("%s",str);
