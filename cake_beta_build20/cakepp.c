...
int analyze(POSITION *p, int color, int how, int depth, double searchtime, int *stat_eval)
	...
	staticeval = evaluation(p,&(si.matcount),-MATE,MATE,&dummy,0,maxNdb);
...
int allscoresearch(SEARCHINFO *si, POSITION *p, MOVE movelist[MAXMOVES], int d, MOVE *best)
	...
	*best = movelist[0];
	...
	for(i=0;i<n;i++)
		...
		values[i] = -negamax(si,p,d-FRAC,-MATE,MATE, &Lkiller, &Lkiller, 0,0,0);
...
int firstnegamax(SEARCHINFO *si, POSITION *p, MOVE movelist[MAXMOVES], int d, int alpha, int beta, MOVE *best)
	...
	*best = movelist[0];
	...
	for(i=0;i<n;i++)
...
#ifndef PVS
		/* normal search: */
		value = -negamax(si, p,d-FRAC,-beta,-Lalpha, &Lkiller, &forcefirst, 0,0,0);
		...
		if(Lalpha==alpha)
			value=-negamax(&p,d-FRAC,-beta,-Lalpha, &Lkiller, &forcefirst,0,0,0);
		else
			{
			/*not in PV - we expect this move to fail low in a minimal window search */
			value=-negamax(&p,d-FRAC,-(Lalpha+1),-Lalpha, &Lkiller, &forcefirst,0,0,0);
			if(value>Lalpha)
...
#endif	
	hashstore(si, p, bestvalue, alpha, beta, d, best, bestindex);
#ifdef THREADSAFEHT
...
int qsearch(SEARCHINFO *si, POSITION *p, int alpha, int beta, int qsdepth)
	...
	// capture for side not to move, or captures if we have to capture
	// alpha is initialized to the static eval of the current position.
	...
	/* what does qs do?
	 * 1) check if we can capture or not
	 * 2) if we can capture, normal recursion over values
	 * 3) if we cannot capture, compute evaluation which serves as "stand-pat-score"
	 *    then get a list of moves which lead to capture for opponent, and recursion over these, maxing value
	 * 4) return the value
	 */

#define QSEARCHLEVEL 300
	...
	if(capture)
		...
		// get stand pat score
		maxvalue = evaluation(p, &(si->matcount), alpha, beta, &delta, 0, maxNdb);
		// if we have made enough qsearches, return evaluation
		//if(qsdepth >= MAXQS)
		//	return maxvalue;
		// if we have an evaluation which is already better than beta, we don't have to do a qsearch
		if(maxvalue >= beta)
			return maxvalue;
		// if we have an evaluation which is far below alpha, we don't have to do a qsearch
		if(maxvalue < alpha-QSEARCHLEVEL)
			return maxvalue;
		// if we get here, then looking for shots makes sense
	...
	for(i=0;i<n;i++)
		...
		//recursion
		value = -qsearch(si, &q, -beta, -alpha, qsdepth+1); 
...
int negamax(SEARCHINFO *si, POSITION *p, int d, int alpha, int beta, int32 *protokiller, int *bestmoveindex, int truncationdepth, int truncationdepth2, int iid)
	/*----------------------------------------------------------------------------
	|																			  |
	...
	if(si->realdepth > MAXDEPTH) 
		...
		si->leaf++;
		si->leafdepth += si->realdepth;
		return evaluation(p,&(si->matcount),alpha,beta,&delta,0,maxNdb);
		}
	...
	if(forcefirst == MAXMOVES-1 && d>IIDDEPTH)
		...
		value = negamax(si, p, d - IIDREDUCE, alpha, beta, protokiller, &forcefirst, truncationdepth, truncationdepth2,1);
...
// or else it's dangerous to call evaluation if side not to move may have a capture
	...
	if(!stmcapture && !isdbpos && !ispvnode)
		...
		localeval = evaluation(p,&(si->matcount),localalpha,localbeta, &delta, stmcapture|sntmcapture,maxNdb);
	...
	if(d<=0 && !stmcapture && !iid && (safemovenum == 0)) // SAFE
		...
		if(!sntmcapture)
			...
			if(localeval != MATE)
				...
				value = evaluation(p,&(si->matcount),localalpha,localbeta,&delta,0,maxNdb);
			...
			if(!isdbpos)
				...
				if(localeval != MATE)
					...
					value = evaluation(p,&(si->matcount),localalpha,localbeta,&delta,1,maxNdb);
	...
	for(i=0;i<n;i++)
		...
		if(bestmovevalue > 128 || stmcapture || i==0 || d<=LATEMOVEMINDEPTH)
			value = -negamax(si, &q,d-FRAC,-beta,-localalpha, &Lkiller, &forcefirst, truncationdepth,truncationdepth2,0);
		else
			{
			value = -negamax(si, &q,d-FRAC-LATEMOVEDEPTH,-beta,-localalpha, &Lkiller, &forcefirst, truncationdepth,truncationdepth2,0);
		...
		if(d-FRAC >=0)
			...
			value = -qsearch(si, &q, -beta, -localalpha, 0);

#else // normal non-qs, nonLMR
		value = -negamax(si, &q, d-FRAC,-beta,-localalpha, &Lkiller, &forcefirst, truncationdepth,truncationdepth2,0);
#endif
		/*************************************************/
	...
	*bestmoveindex = bestindex;
	...
	if(d>=0)
		...
		EnterCriticalSection(&hash_access);
#endif	
		hashstore(si, p, maxvalue, alpha, beta, d, &movelist[bestindex], bestindex); 
#ifdef THREADSAFEHT
		LeaveCriticalSection(&hash_access);
#endif	
		}
...
void hashstore(SEARCHINFO *si, POSITION *p, int value, int alpha, int beta, int depth, MOVE *best, int32 bestindex)
	{
...
void getpv(SEARCHINFO *si, POSITION *p, char *str)
	...
	for(i=0;i<MAXDEPTH;i++)
		...
		staticeval = evaluation(&Lp,&(si->matcount),-MATE,MATE,&dummy,0,maxNdb);
...
// TODO: copy stuff from crafty for 64-bit windows!
