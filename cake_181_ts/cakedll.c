...
// cake.dll is not thread-safe yet because it uses extern SEARCHINFO si!
// todo: get an own allscores global which can be queried from getmove on - or pass it as
// parameter 
...
int __stdcall WINAPI enginecommand(char str[256], char reply[256])
	...
	if(strcmp(command,"staticevaluation")==0)
		...
		sprintf(reply, "evaluation from point of view of side to move is:\nTotal:\t%i\nMaterial:\t%i\nMen:\t%i\nBack Rank:\t%i\nRunaway:\t%i\nCramp:\t%i\nHold:\t%i\nKing:\t%i\nSelftrap:\t%i\nKing-Man:\t%i\nCompensation:\t%i",staticeval, e.material, e.men, e.backrank, e.runaway, e.cramp, e.hold, e.king, e.selftrap, e.king_man, e.compensation);
		//sprintf(reply,"static evaluation is: \nTotal: %i\nMaterial: %i\nPositional: %i\nError: %i",total, material, positional,delta);
		return 1;
		}
